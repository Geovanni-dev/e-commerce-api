...
const productSchema = z.object({
    ...
    price: z.preprocess((val) => Number(val), z.number().positive("O preço deve ser maior que zero")),
    stock: z.preprocess((val) => parseInt(val), z.number().int().nonnegative("O estoque não pode ser negativo")),
...
const querySchema = z.object({
    ...
    minPrice: z.preprocess((val) => Number(val), z.number().positive()).optional(),
    maxPrice: z.preprocess((val) => Number(val), z.number().positive()).optional(),
    limit: z.preprocess((val) => parseInt(val), z.number().int().positive()).default(50),
    page: z.preprocess((val) => parseInt(val), z.number().int().positive()).default(1),
    order: z.enum(['preco_asc', 'preco_desc', 'nome_asc', 'nome_desc']).optional()
});
...
const listAllProducts = async (req, res) => {
    try {
        ...
        orderBy: // ordenaçao de produtos ex; asc (0-9), desc (9-0), name (a-z), name (z-a)
            validatedProduct.order === 'preco_asc' ? { price: 'asc' } :  // do menor preço para o maior
            validatedProduct.order === 'preco_desc' ? { price: 'desc' } : // do maior preco para o menor
            validatedProduct.order === 'nome_asc' ? { name: 'asc' } : // de a-z
            validatedProduct.order === 'nome_desc' ? { name: 'desc' } /* de z-a*/ : undefined,
...
const listProduct = async (req, res) => {
    try {
        ...
        const product = await prisma.product.findUnique({
            ...
            include: { 
                category: true }}); // Funçao para mostrar as categorias no produto
            
            if (!product) { // Para caso o cliente digite um id n existente
