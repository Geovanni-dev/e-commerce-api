...
const createOrder = async (req, res) => {
  ...
  try {
    ...
    await Promise.all(
      // usei promise.all para diminuir o estoque de todos os produtos ao mesmo tempo
      cart.items.map((item) =>
...
const listOrder = async (req, res) => {
  ...
  try {
    ...
    if (!order) {
      // se o pedido não existir ou n for encontrado
      return res.status(404).json({ error: 'Pedido n encontrado' });
    }
    if (order.userId !== req.user.id) {
      // se o pedido n pertencer ao user logado
