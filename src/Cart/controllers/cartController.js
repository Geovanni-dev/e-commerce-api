...
const addItemCart = async (req, res) => {
  try {
    ...
    const cart = await prisma.cart.findUnique({
      // buscando o carrinho
      where: {
        // buscando o carrinho associado ao usuario logado
        userId: req.user.id,
      },
    });
    if (!cart) {
      // se o carrinho n for encontrado
...
const editItemCart = async (req, res) => {
  try {
    ...
    const cartItem = await prisma.cartItem.findUnique({
      // buscando o produto e verificando se pertense ao arrinho do user
      where: {
        id,
      },
      include: {
        // usei include para mostrar o carrinho junto com o produto
        cart: true,
      },
    });
    if (!cartItem) {
    ...
    if (cartItem.cart.userId !== req.user.id) {
      //verifica se o produto pertence ao carrinho do user
...
const removeItemCart = async (req, res) => {
  try {
    ...
    if (cartItem.cart.userId !== req.user.id) {
      ...
      return res
        .status(403)
        .json({ error: 'Produto não pertence ao carrinho' });
    }
