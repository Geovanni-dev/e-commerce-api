...
const register = async (req, res) => {
  try {
    ...
    const passwordHash = await bcrypt.hash(password, 10); // criptografando a senha
    ...
    await emailCode(
      user.email, //função para enviar o email com o codigo de verificação
  ...
  try {
    ...
    const user = await prisma.user.findFirst({
      ...
      where: {
        email,
        verificationCode: code,
      }, // recebe o codigo validado
    });
    if (!user) {
      // se o usuario n for encontrado
      return res
        .status(404)
        .json({ error: 'Codigo invalido ou e-mail não encontrado' });
    }
    await prisma.user.update({
      // impede que o codigo seja usado mais de uma vez
      where: { id: user.id },
...
// função assincrona para logar o usuario
const login = async (req, res) => {
  try {
    ...
    const passwordValid = await bcrypt.compare(
      password,
      user.password,
    ); /* comparando a senha digitada com a senha do banco de dados*/
    if (!passwordValid) {
      // se a senha n for igual
      return res.status(401).json({ error: 'Email ou senha incorretos' });
    }
    if (!user.verified) {
      // se o usuario n for verificado
      return res
        .status(403)
        .json({ error: 'Verifique seu email antes de fazer loguin' });
    }
    ...
    const secret = jwt.sign({ id: user.id }, process.env.JWT_SECRET, {
      expiresIn: '1d',
    }); // criando o token
    ...
    if (error instanceof z.ZodError) {
      ...
      return res.status(400).json({
        error: 'Erro de validação',
        detalhes: error.flatten().fieldErrors, // imprimir os erros
      });
    }
    if (error.code === 'P2002') {
      // se o erro for do prisma, indicando que o email ja existe no banco de dados
...
const listAllUsers = async (req, res) => {
  try {
    ...
    const users = await prisma.user.findMany({
      // buscando os usuarios no banco de dados
      where: {
        ...
        name: name
          ...
          : undefined, // serve para n diferenciar maiusculas e minusculas
        email,
        role,
      },
      select: {
...
const forgetPassword = async (req, res) => {
  // funcao assincrona para esquecer a senha
  try {
    ...
    const resetLink = `${process.env.RESET_PASSWORD_URL}/${resetToken}`; // cria o link de recuperação
    try {
      await emailCode(
        email,
...
const resetPassword = async (req, res) => {
  // funcao assincrona para trocar a senha
  try {
    ...
    if (!user) {
      ...
      return res
        .status(404)
        .json({ error: 'Token de recuperação de senha inválido' });
    }
